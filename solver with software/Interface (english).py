...
Time_txt = open("Time_txt.txt", "r")
Times = Time_txt.readlines()
Time_txt.close()
Time_List = [int(str_time[:2])*60 + int(str_time[3:5]) + int(str_time[6:8])/100 for str_time in Times]
canvasCourbe = Canvas(frameImage, width = 660, height = 420, bg = '#C2BCBA', highlightthickness = 0)
courbe1 = Image.open("Curve.png")
Courbes = ImageTk.PhotoImage(courbe1)
img = canvasCourbe.create_image(330, 200, image = Courbes)
canvasCourbe.pack()
...
def Courbe(Y):
    ...
    courbe1 = Image.open("Curve.png")
    Courbes = ImageTk.PhotoImage(courbe1)
    canvasCourbe.itemconfigure(img, image = Courbes)
    canvasCourbe.image = Courbes
    canvasCourbe.update()
...
labelFrameCamera = Label(frameTitreCamera, text = "Times curve",   bg = '#C2BCBA')
labelFrameCamera.configure(font=fontConflabelFrameCamera)
labelFrameCamera.pack()

#===========================================================================================#
#Onglet control curve
def Ouvrir_Fermer(etat):
    ...
    if open_closeCam == 1:
        ...
        ouvrir_fermer.config(text = "Display curve")
    else:
...
def Resolution():
    ...
    s, _ = SolveBH.solve(SolveBH.getCube())
    #envoie(s)
    print(s)
    SolveBH.resetCube()
    updateCube(SolveBH.getCube())
...
def UpdateTimer(e):
    ...
    #Génération et envoie du mélange

    '''GenererMelange()
    
    SolveBH.setCube(outils.applyMove(SolveBH.getCube(), mel))
    updateCube(SolveBH.getCube())
...
def ResetTimer(e):
    ...
    Time_txt = open("Time_txt.txt", "a")
    Time_txt.write(str_time + "\n")
    Time_txt.close()
    time_in_second = int(str_time[:2])*60 + int(str_time[3:5]) + int(str_time[6:])/100
    Time_List.append(time_in_second)
    Courbe(Time_List)
...
def Reset_Last():
    global Time_List
    Time = open("Time_txt.txt", "r")
    lignes = Time.readlines()
    lignes = lignes[:-1]
    Time_List = Time_List[:-1]
    Time.close()
    Time = open("Time_txt.txt", "w")
    for i in lignes:
        Time.write(i)
    Time.close()
    Courbe([int(str_time[:2])*60 + int(str_time[3:5]) + int(str_time[6:8])/100 for str_time in lignes])
reset = Button(frameImage, text = 'Reset last', command = Reset_Last)
reset.pack(side = LEFT, padx = 10)

def Reset_all():
    global Time_List
    Time_List = []
    Time = Time = open("Time_txt.txt", "w")
    Time.close()
    Courbe(Time_List)

resetall = Button(frameImage, text = 'Reset all', command = Reset_all)
